...
#----------------------------------------------------------------------------
my $model = {
    title => 'KeTech LED Font Testbed 2',
    version => "v2.0 (11th Nov 2013)",
    mw => undef,
    c => undef,
    colours => {
        bg => "#1e501d",
    ...
    tcp=>{
        host => 'localhost',
        port => 49333,
        bridge => "C:\\projects\\eclipse-workspace\\test-jssc-1\\tcpser.exe",
        seriallink => "COM4",
    },
    geom => {
        gridx => 5,
...
sub save_config {
    my $filename = shift;
    my %conf;
    my @keys = qw( geom tcp colours );
    foreach(@keys){
        $conf{$_} = $model->{$_};
    }
    YAML::DumpFile($filename, $model->{title}, $model->{version}, \%conf);

}
# save_config("testconfig.txt");
# exit;
...
sub make_gui
{
    ...
    my $c = $mw->Scrolled('Canvas', -width => 800, -height => 480, 
        -bg => $clr->{bg},    -relief       => 'sunken',
        -borderwidth  => 2,
            -scrollbars   => 'osoe',
...
sub bridge_launch
{
    ...
    my $m = $model;
    my $cmd = $m->{tcp}->{bridge};
    # todo args for serial and TCP ports
    my $tcp_port = $m->{tcp}->{port};
    my $serial_port = $m->{tcp}->{seriallink};
    my $status =  system($cmd, $serial_port, $tcp_port);
    warn "cmd exited funny: $?" unless $status == 0;
    t "bridge launch finish";
}
...
sub  draw_grid {
    ...
    my $cellw = $g->{gridx};
    my $cellh = $g->{gridy};
    my $numrows = $g->{numrows};
    my $numcols = $g->{numcols};
...
#              ['command', "Clear", -command => \&journey_clear],
          ...
          ['~Edit',
            [
              ...
              [command => 'Set grid geometry', -command =>sub{set_grid_geometry()}],
